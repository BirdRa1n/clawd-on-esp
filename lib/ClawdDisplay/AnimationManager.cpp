...
static fs::FS *gFs = nullptr;   // asset filesystem (LittleFS or SD)
...
bool AnimationManager::begin(TFT_eSPI *tft, fs::FS *assetFs) {
  _tft = tft;
  gTft = tft;
  gFs = assetFs;
  tft->setSwapBytes(true);   // our palette is RGB565 LE; TFT wants byte-swapped on the wire
  _assetsOk = gFs && gFs->exists("/idle.crli");
  console.logf("[anim] assets %s", _assetsOk ? "present" : "MISSING");
  return _assetsOk;
}
...
bool AnimationManager::openState(ClawdState s) {
  ...
  if (need > runbufCap) {
    ...
    if (!runbuf) { console.log("[anim] runbuf alloc failed"); cf.close(); return false; }
  }
  ...
  fFrameIdx = 0;
  _open = true;
  console.logf("[anim] %s: %dx%d %df pal%d maxRuns%d\n", path, fW, fH, fFrames, fPalN, fMaxRuns);
  return true;
}
